...
const UsersPage: React.FC<UsersPageProps> = ({ fullTable }) => {
	// State for all users
			...
			<div style={{ maxWidth: 900, margin: "2rem auto", color: "#fff", background: '#232a3b', borderRadius: 12, boxShadow: '0 4px 24px #0002', padding: '2.5rem 2rem', width: '95vw', minWidth: 0 }}>
				{/* Page title */}
								<h2 style={{ color: '#4f8cff', fontWeight: 700, marginBottom: 24 }}>Users Management</h2>
												{!fullTable && (
													<button
														style={{ background: '#4f8cff', color: '#fff', border: 'none', borderRadius: 6, padding: '0.5rem 1.2rem', fontWeight: 700, marginBottom: 16 }}
														onClick={() => navigate('/users/table')}
													>View Table</button>
												)}
												{fullTable && (
													<button
														style={{ background: '#232a3b', color: '#4f8cff', border: '1px solid #4f8cff', borderRadius: 6, padding: '0.5rem 1.2rem', fontWeight: 700, marginBottom: 16 }}
				...
				<div className="table-container">
				  {loading ? (
					  ...
					  <table style={{ width: fullTable ? '360px' : '100%', borderCollapse: 'collapse', background: '#181c23', borderRadius: 8, fontSize: fullTable ? '0.92rem' : undefined }}>
						  <thead>
							  <tr style={{ color: '#4f8cff', fontWeight: 700 }}>
								  ...
								  {(fullTable ? true : false) && <th style={{ padding: '0.7rem' }}>Password</th>}
								  {(fullTable ? true : false) && <th style={{ padding: '0.7rem' }}>Role</th>}
								  {(fullTable ? true : false) && <th style={{ padding: '0.7rem' }}>Enrollments</th>}
								  {(fullTable ? true : false) && <th style={{ padding: '0.7rem' }}>Actions</th>}
							  </tr>
						  </thead>
						  <tbody>
							  {users.map((user: any) => (
								  <tr key={user.StudentID} style={{ background: '#181c23', borderRadius: 8, boxShadow: '0 2px 8px #0001' }}>
									  ...
									  {fullTable && <td style={{ padding: '0.9rem 0.7rem' }}>{user.Password}</td>}
									  {fullTable && <td style={{ padding: '0.9rem 0.7rem' }}>{user.Role}</td>}
									  {fullTable && <td style={{ padding: '0.9rem 0.7rem', textAlign: 'center' }}>
									  ...
									  {fullTable && <td style={{ padding: '0.9rem 0.7rem', textAlign: 'center', borderRadius: '0 8px 8px 0' }}>
